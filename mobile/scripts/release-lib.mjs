...
export function validateEvidence(evidence, { platform, kind, digest, now = Date.now() }) {
  ...
  if (!p || !/^\d+$/.test(p.buildNumber || "") || !/^[a-f0-9]{64}$/.test(p.artifactSha256 || "") || typeof p.device !== "string" || !p.device.trim() || typeof p.osVersion !== "string" || !p.osVersion.trim()) throw new Error("Exact signed artifact, build number and physical device are required");
