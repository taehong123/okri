...
function DailySubmissionSummary({ submission, onOpenTask, onOpenWork }: { submission: DailySubmission; onOpenTask: (id: string) => void; onOpenWork: (work: DailyWork) => void }) {
  ...
  return <div className="daily-submission-summary">
    <b>{t("완료한 일")}</b>
    <ul aria-label={t("완료한 일")}>{completedWork.length ? completedWork.map((entry) => <li key={entry.key}><button onClick={() => onOpenWork(entry)}>{entry.title}</button><small>{entry.kind === "project" ? t("Project") : entry.kind === "task" ? t("Task") : t("Routine")} · {entry.parentTitle}</small></li>) : <li>{t("선택한 업무 없음")}</li>}</ul>
