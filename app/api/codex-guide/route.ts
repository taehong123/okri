...
const guide = {
  ...
  endpoints: [
    ...
    { purpose: "List deleted Project, Ticket and Task roots", method: "GET", path: "/api/item-trash" },
    { purpose: "Move selected Project, Ticket and Task items to trash", method: "POST", path: "/api/item-trash", body: "itemIds, or the backward-compatible scope=all_project_task for every active Project, Ticket and Task in the current workspace", requiresConfirmation: true },
    { purpose: "Restore Project or Ticket bundles, or independently deleted Tasks", method: "PATCH", path: "/api/item-trash", body: "itemIds and action=restore" },
    { purpose: "Permanently delete trashed Project or Ticket bundles, or Tasks", method: "DELETE", path: "/api/item-trash", body: "itemIds and confirmationText=영구 삭제", requiresConfirmation: true },
  ...
  behavior: [
    ...
    "Classify by completion boundary: one action is Task, an incoming request or issue is Ticket, a finite deliverable containing independent Tasks is Project, recurring execution is Routine. Ask one short question when ambiguous; respect the user's choice.",
    "Use General only after the user explicitly chooses it, or when the server confirms that no active Project/Ticket/Routine candidate exists. Missing IDs do not authorize General. A Project without its Initiative remains an unsaved conversation draft; never invent ancestors or silently downgrade it.",
