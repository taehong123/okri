...
import { deliverStoreReviewEvent, parseAppleReviewEvent, verifyStoreWebhookSignature, type StoreReviewRuntime } from "@/lib/store-review-feedback";
...
export async function POST(request: Request) {
  ...
  const runtime = env as unknown as StoreReviewRuntime & { APPLE_STORE_WEBHOOK_SECRET?: string };
  if (!await verifyStoreWebhookSignature(rawBody, request.headers.get("x-apple-signature"), runtime.APPLE_STORE_WEBHOOK_SECRET)) {
    return Response.json({ error: "Invalid signature" }, { status: 401 });
  }
  const payload = await parseJson(rawBody);
  if (!payload) return Response.json({ error: "Invalid JSON" }, { status: 400 });
  const event = await parseAppleReviewEvent(payload, rawBody);
