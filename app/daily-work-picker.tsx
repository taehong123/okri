"use client";
import { ChevronDown, ChevronRight, LoaderCircle, Plus, Search, X } from "lucide-react";
...
export function DailyWorkPicker({ label, work, selected, disabled, noPlanned, yesterday = false, conflictKeys = [], onChange, onNoPlanned, onOpen, projects = [], onCreate }: {
  ...
  onCreate?: (projectId: string, title: string, requestId: string) => Promise<boolean>;
}) {
  ...
  function closeEntry() { setCreating(null); requestAnimationFrame(() => addButton.current?.focus()); }
  ...
  async function createTask() {
    if (!creating?.title.trim() || !onCreate || busy) return;
    setBusy(true);
    try { if (await onCreate(creating.projectId, creating.title, creating.requestId)) closeEntry(); }
    ...
    <div className="daily-picker-panel">
      ...
      <div className="daily-task-groups">{groups.map((group) => {
        ...
        {creating && creating.projectId === group.projectId && <form className="daily-project-create" aria-label={t("Task 추가")} onSubmit={(event) => { event.preventDefault(); void createTask(); }}>
          <label><span>{t("새 Task 제목")}</span><input ref={titleInput} value={creating.title} disabled={busy} maxLength={240} onKeyDown={closeOnEscape} onChange={(event) => setCreating({ ...creating, title: event.target.value })} /></label>
          <div><button className="secondary" type="submit" disabled={busy || disabled || !creating.title.trim()} aria-busy={busy} onKeyDown={closeOnEscape}>{busy ? <LoaderCircle size={16} className="spin" /> : <Plus size={16} />}{t("추가하고 오늘 할 일에 선택")}</button><button className="icon-button" type="button" disabled={busy} title={t("취소")} aria-label={t("취소")} onClick={closeEntry} onKeyDown={closeOnEscape}><X size={16} /></button></div>
        </form>}
        </section>;
      })}</div>
