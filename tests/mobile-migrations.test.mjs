...
test("native tables are created after an already-applied 0055 migration and a second run preserves them", async t => {
  ...
  db.prepare("INSERT INTO __drizzle_migrations (hash,created_at) VALUES (?,?)").run("already-published-0055", 1788840000000);
  ...
  for (const table of ["native_sessions", "native_auth_codes", "native_apple_nonces", "native_apple_grants", "native_identity_revocations", "native_account_deletion_guards"]) {
...
test("native repair preserves the original schema and newly appended migration timestamps exceed history", async () => {
  const repair = (await read("drizzle/0056_native_sessions_repair.sql")).replace(/^--[^\n]*\n/, "");
  assert.equal(repair, await read("drizzle/0054_native_sessions.sql"));
